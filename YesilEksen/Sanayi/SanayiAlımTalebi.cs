...
namespace YesilEksen
{
    public partial class SanayiAlımTalebi : Form
    {
        ...
        private string currentFirmaDurum = "";
        private string currentCiftlikDurum = "";
        ...
        private void LoadTalepDetay(int talepID)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    // Durumları sakla
                    currentFirmaDurum = row["FirmaDurumAdi"]?.ToString() ?? "";
                    currentCiftlikDurum = row["CiftlikDurumAdi"]?.ToString() ?? "";
                    
                    // Durum labellarını güncelle
                    label8.Text = $"Durumu > {currentFirmaDurum}";
                    label8.ForeColor = currentFirmaDurum == "Onaylandı" ? Color.Green : Color.Red;
                    
                    label9.Text = $"Durumu > {currentCiftlikDurum}";
                    label9.ForeColor = currentCiftlikDurum == "Onaylandı" ? Color.Green : Color.Red;
        ...
        /// <summary>
        /// Onayla ve Reddet butonlarının durumunu günceller
        /// </summary>
        private void UpdateButtonStates()
        {
            bool canProcess = CanProcessRequest();

            button1.Enabled = canProcess; // Onayla butonu
            button3.Enabled = canProcess; // Reddet butonu

            if (!canProcess)
            {
                // Butonlar devre dışı iken tooltip benzeri bilgi verebiliriz
                // Ancak şimdilik sadece devre dışı bırakıyoruz
            }
        }

        /// <summary>
        /// Talebin işlenip işlenemeyeceğini kontrol eder
        /// </summary>
        /// <returns>İşlem yapılabilirse true, aksi halde false</returns>
        private bool CanProcessRequest()
        {
            // Kural 1: Çiftlik durumu "Onay Bekliyor" ise işlem yapılamaz
            if (currentCiftlikDurum == "Onay Bekliyor")
            {
                return false;
            }

            // Kural 2: Çiftlik "Onaylandı" ama firma "Onay Bekliyor" ise işlem yapılamaz
            if (currentCiftlikDurum == "Onaylandı" && currentFirmaDurum == "Onay Bekliyor")
            {
                return false;
            }

            // Kural 3: Çiftlik durumu "Onaylandı" olmalı (firma durumu da "Onaylandı" olmalı)
            if (currentCiftlikDurum == "Onaylandı" && currentFirmaDurum == "Onaylandı")
            {
                return true;
            }

            // Diğer durumlarda işlem yapılamaz
            return false;
        }
        ...
        private void BtnOnayla_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Durum kontrolü - İşlem yapılabilir mi?
        ...
        /// <summary>
        /// İşlem yapılamaz durumda uyarı mesajı gösterir
        /// </summary>
        private void ShowCannotProcessMessage()
        {
            string message = "Bu talep üzerinde işlem yapılamaz!\n\n";

            if (currentCiftlikDurum == "Onay Bekliyor")
            {
                message += $"• Çiftlik durumu '{currentCiftlikDurum}' durumunda.\n" +
                          "• Çiftlik durumu 'Onaylandı' olmalıdır.";
            }
            else if (currentCiftlikDurum == "Onaylandı" && currentFirmaDurum == "Onay Bekliyor")
            {
                message += $"• Çiftlik durumu '{currentCiftlikDurum}' ✓\n" +
                          $"• Ancak firma durumu '{currentFirmaDurum}' durumunda.\n" +
                          "• Hem çiftlik hem de firma durumu 'Onaylandı' olmalıdır.";
            }
            else
            {
                message += $"• Çiftlik durumu: {currentCiftlikDurum}\n" +
                          $"• Firma durumu: {currentFirmaDurum}\n" +
                          "• Her iki durumun da 'Onaylandı' olması gerekmektedir.";
            }

            MessageBox.Show(message, "İşlem Yapılamaz", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
